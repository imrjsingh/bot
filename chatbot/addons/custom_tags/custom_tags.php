...
function parse_wiki_tag($convoArr, $element, $parentName, $level)
{
    ...
    $wikiURL = 'http://es.wikipedia.org/w/api.php?action=opensearch&format=xml&limit=1&search=' . urlencode($response_string);
    $options = array(
    ...
    if ((string)$xml->Section->Item->Description)
    {
        ...
        $output = "$description Si quieres leer más lo tienes todo aquí ".$linkHref;
    }
    else {
